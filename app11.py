...
/* Only border containers that are nested (inside columns or other containers) */
...
/* Top-level page wrapper: no border, transparent */
...
/* Column wrappers inside stHorizontalBlock must NOT have the card border
   (those are layout containers, not chart panels) */
...
.stn-card.selected { border-color: #2563eb; box-shadow: 0 0 0 2px #2563eb33; }
...
.nd-bdg   { background: #f7fafc; color: #a0aec0; border: 1px solid #e2e8f0;
            font-size: 9px; font-weight: 700; padding: 2px 7px; border-radius: 4px; }
.click-hint { font-size: 10px; color: #2563eb !important; margin-top: 8px;
...
# Session state: station được click
...
if tab == "🏠 Dashboard":
    ...
    n_bad = int((df_sf.groupby("station_name")["AQI"].mean().dropna() > 100).sum())
    ...
    <div class="kpi-row">
      ...
      <div class="kpi-card">
        <div>
          ...
          <span class="kpi-bdg" style="color:#dc2626;border-color:#dc2626;background:rgba(220,38,38,.1)">AQI &gt; 100</span>
        </div>
  ...
  <div class="stn-row">
    ...
    # Render 3 cards per row, each with a button below
...
# ══════════════════════════════════════════════
    ...
    if eda_mode == "🏙️ Toàn thành phố":
        ...
        # Pre-select from clicked button on Dashboard
        default_idx = 0
...
# ──────────────────────────────────────────────
st.markdown("""
<div style="text-align:center;padding:8px 0 4px;margin-top:4px;
