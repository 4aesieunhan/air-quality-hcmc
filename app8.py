...
if tab == "🏠 Dashboard":
    ...
    st.markdown("_Click **Xem EDA** để phân tích chi tiết từng trạm_", unsafe_allow_html=False)
    ...
    for i in range(0, len(stations), 3):
        ...
        for co, stn in zip(cols, batch):
            with co:
                ...
                if st.button(f"📊 Xem EDA – {stn[:25]}", key=f"btn_{stn}",
                             use_container_width=True):
                    st.session_state["eda_station"] = stn
                    st.rerun()
